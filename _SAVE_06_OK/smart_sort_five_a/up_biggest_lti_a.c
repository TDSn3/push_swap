...
/*   Updated: 2022/06/10 14:04:24 by tda-silv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    up_biggest_lti_a(t_data *d, t_tli *tli, t_tli *firstpart_stock)
{
	...
	if (firstpart_stock)
	{
		while (i < d->size_a && d->stack_a[i] != find_pos_max_tli(firstpart_stock)->content)
			i++;
		if (d->stack_a[0] != find_pos_max_tli(firstpart_stock)->content)
		{
			if (pos_in_stack(d->stack_a, d->size_a, d->stack_a[i]) > d->size_a / 2)
			{
				while (d->stack_a[0] != find_pos_max_tli(firstpart_stock)->content && j < d->size_a)
				{
					rra(d);
					j++;
				}
