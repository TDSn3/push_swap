...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   sfd_make_all_oper_b_r.c                              :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: tda-silv <tda-silv@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/05/30 16:10:52 by tda-silv          #+#    #+#             */
/*   Updated: 2022/06/20 15:03:51 by tda-silv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	sfd_make_all_oper_b_r(t_double_lst *dl, t_data *d, t_sub_lst **lst_oper)
{
	...
	while (copy_sl)
	{	
		if (sfd_if_sorted_b_r(d, copy_sl, lst_oper))
			return ;
	...
	sfd_make_all_oper_b_r(dl, d, lst_oper);
}
...
static void	next_to_sa(t_data *d, t_double_lst *dl, t_sub_lst *copy_sl)
{
	...
	td = ra_dt(copy_sl->stack_after_oper, d->size_b);
	if (!check_duplicate_b(dl, d, td) && copy_sl->nb_rra < 5)
	{
		td = ra_dt(copy_sl->stack_after_oper, d->size_b);
	...
	td = rra_dt(copy_sl->stack_after_oper, d->size_b);
	if (!check_duplicate_b(dl, d, td) && copy_sl->nb_rra > 0)
	{
		td = rra_dt(copy_sl->stack_after_oper, d->size_b);
...
static void	next_to_ra(t_data *d, t_double_lst *dl, t_sub_lst *copy_sl)
{
	...
	td = sa_dt(copy_sl->stack_after_oper, d->size_b);
	if (!check_duplicate_b(dl, d, td) && copy_sl->nb_rra < 4)
	{
		td = sa_dt(copy_sl->stack_after_oper, d->size_b);
