...
/*   Updated: 2022/06/08 20:18:30 by tda-silv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	next(t_data *d)
{
	...
	while (d->size_a > i)
	{
		...
		up_biggest_lti_a(d, tli_a, firstpart_stock);
...
//		sort_first_five_a(d);
		...
		sort_five_double(d);
		firstpart_stock = tli_cpy(stock_tli_a, firstpart_stock);
	}
	while (d->size_b > 0)
		pa(d);

	tli_clear(&stock_tli_a);
	tli_clear(&firstpart_stock);
}
